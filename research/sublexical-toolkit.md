...
The Sublexical Toolkit is an open source language analysis package I lead development on at the [Neural Tuning of Reading Lab](https://sites.google.com/view/www-jjpurcell-com/home/about-me) at the University of Maryland. It computes over 80 derived linguistic measures for 22,474 English words, analyzing the consistency of spelling-sound relationships across syllabic positions and multiple grain sizes. I work on this project with a team of undergraduate researchers under Dr. Jeremy Purcell.

## Data Pipeline

The toolkit's data pipeline has two stages. The first stage, which I built in Python, handles data collection and quality assurance: webscraping dictionary data from multiple accredited sources, deduplicating entries, resolving discrepant pronunciations, and producing a validated master vocabulary list. The second stage processes the master list through R-based linguistic analysis, mapping every word through the Maximum Onset Principle for syllabification, computing probability tables at each syllabic position, and extracting the full set of derived measures.

![Sublexical Toolkit Data Pipeline](/assets/images/toolkit/toolkit_pipeline.png)
...
![Dataset Scale](/assets/images/toolkit/toolkit_dataset_growth.png)
...
![Four Grain Sizes](/assets/images/toolkit/toolkit_grain_sizes.png)
...
![Syllabic Position Consistency](/assets/images/toolkit/toolkit_syllabic_positions.png)
...
![Measure Taxonomy](/assets/images/toolkit/toolkit_measures.png)
...
**Python data validation pipeline**: I built a suite of quality assurance scripts including duplicate detection across multiple datasets (`cdupes.py`), cross-dataset comparison (`compare_excel.py`, `coldiff.py`), and automated measure description generation from column naming patterns (`toolkit-descriptions.py`).
...
I lead the software development team for this project. My contributions include building the entire Python data pipeline from scratch, scaling the dataset, designing the validation and QA tooling, running correlation analyses for publications (including work with Stanford ROAR reading assessment data), refactoring the backend, and presenting the NTR Orthogonalization Library at the 2023 Society for Neuroscience conference.
